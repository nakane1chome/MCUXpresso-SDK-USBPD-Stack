    ...
    (defined(PD_CONFIG_SRC_AUTO_DISCOVER_CABLE_PLUG) && (PD_CONFIG_SRC_AUTO_DISCOVER_CABLE_PLUG))
                ...
                case PSM_PE_SNK_DISCOVERY: /* (B) */
                {
                    ...
                    else if (pdInstance->psmHardResetNeedsVSafe0V != 0U)
                    {
                        ...
                        else if (PD_PsmCheckVsafe0V(pdInstance) != 0U)
                        {
                            pdInstance->psmHardResetNeedsVSafe0V = 0U;
                        }
                        else
                    ...
                    case kVDM_DiscoverIdentity:
                        if (triggerInfo->vdmHeader.bitFields.SVID == PD_STANDARD_ID)
                        {
                            ...
                            if (commandVdmRequest.requestResultStatus == (uint8_t)kCommandResult_VDMACK)
                            {
                                if (pdInstance->revision < PD_SPEC_REVISION_30)
                                {
                                    /* PD2.0 spec: these fields should be 0 */
                                    pd_id_header_vdo_t *idHeaderVdo =
                                        (pd_id_header_vdo_t *)((void *)commandVdmRequest.vdoData);

                                    idHeaderVdo->bitFields.productTypeDFP = 0;

                                    if ((idHeaderVdo->bitFields.productTypeUFPOrCablePlug ==
                                         (uint8_t)kProductTypeUfp_PSD) ||
                                        (idHeaderVdo->bitFields.productTypeUFPOrCablePlug ==
                                         (uint8_t)kProductTypeUfp_VPD))
                                    {
                                        idHeaderVdo->bitFields.productTypeUFPOrCablePlug = 0;
                                    }
                                }
                            }
                        }
...
pd_status_t PD_Control(pd_handle pdHandle, uint32_t controlCode, void *param)
{
    ...
    switch ((pd_control_t)controlCode)
    {
        case PD_CONTROL_GET_PD_STATE:
            /* Timer busy, Event pending, Command processing, Type-C not stable, PD state machine not stable. */
            /* 1. Timer runing, event pending or command process indicates PD or Type-C state machine is busy. */
            /* 2. Type-C state machine is not stable */
            /* 3. Ready state indicates PD stack has completed PD negotiation. */
            /* 4. SrcDisabled state indicates partner sink doesn't reply GoodCRC in response to SrcCapability message.
                  There isn't PD establishment or partner sink doesn't support PD communication. */
            /* 5. SnkWaitCap state indicates self sink doesn't receive SrcCapability message.
                  There isn't PD establishment or partner source doesn't support PD communication. */
